<template>
	<div class="navTop" :style="{ 'max-width': bodyW }">
		...
		<div class="explainCard ruleBody">
			<div class="ruleContent" id="ruleCon"></div>
		</div>
			...
			roll() {
				...
				if (luck.times > luck.cycle + 10 && luck.prize == luck.index) {
					...
					if (luck.times < luck.cycle) {
						luck.speed -= 10;
					} else if (luck.times == luck.cycle) {} else {
					...
					function(res) {
						if (res.err_code != 800) {
							if (res.err_code == 0) {
								...
								common.toAjax(common.host + '/prizes/list', {
			...
			common.toAjax(common.host + '/prizes/latest_prized', {}, function(res) {
				if (res.err_code != 800) {
					if (res.err_code == 0) {
						...
						function plane() {
							var name1 = (that.prizedList[count].name ? that.prizedList[count].name : that.prizedList[count].tel).substring(
								0, 5);

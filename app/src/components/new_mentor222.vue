	...
	<div class="navTop" :style="navTop">
				...
				<div :style="step_box" class="step_box" v-if="stepList == 5">
					...
					<ul :style="ulStyle" class="ulStyle">
						<li v-for="stepIndex in stepList" :key="stepIndex" :class="stepList <= 3 ? 'step3' : 'step5'">
							...
							<img src="https://qiniustore.zmr016.com/invite/money-1.png" alt="" class="currentMoney" :style="currentMoney" v-if="stepIndex == stepNum" />
		...
		<div v-if="chooseTabIndex == 1">
				...
				<van-list v-model="loading" :finished="finished" finished-text="没有更多了" @load="onLoad" :immediate-check="true">
					<van-row class="friend_list" v-for="(item, index) in list" :key="index">
						<van-col span="10" style="text-align: left;padding-left: 10px;">
							...
							<span style="display: inline-block;vertical-align: middle;width:60%;overflow: hidden;white-space: nowrap;text-overflow: ellipsis;">
								{{ item.name }}
							</span>
						</van-col>
			...
			<div class="centerBox noData" :style="centerBox" v-if="noData">
				<div class="homeTab2_noFriend">
					<div class="nofriend_icon"><p>暂时没有好友哦~</p></div>
					<div class="nofriend_btn" @click="commonShare">赶紧去邀请好友吧</div>
				</div>
			</div>
		</div>
		<!-- tab内容卡 我的好友 end -->
		...
		<div v-if="chooseTabIndex == 2">
			<div class="centerBox paddingBottom" :style="centerBox">
				...
				<div
					style="font-size: 14px;text-align: center;width:100%;height: 30px;line-height:30px;background:#f1f1f1 ;color: #999999;display: block;"
		...
		<van-popup v-model="show" :overlay="true" :close-on-click-overlay="ermShow ? true : false" :style="ermShow ? 'background-color:transparent' : ''" @closed="ermShow = false">
			<!-- 宝箱弹窗 -->
			<div class="pop_div" id="getCoin" v-if="isCoinShow">
				...
				<p><span class="button" @click="shareCoin">分享到微信好友，再得50金币</span></p>
			</div>
			<!-- 宝箱弹窗 end -->
			<!-- 奖励卡弹窗1-5 -->
			<div class="pop_div price_notice_bg" id="priceNotice" v-if="isPriceShow">
				...
				<p><span class="button" @click="priceShare">立即邀请好友</span></p>
			</div>
			<!-- 奖励卡弹窗1-5 end -->
			<!-- 活动规则按钮和弹窗 -->
				...
				<div style="width: 100%;height: 94%;padding: 3% 0;overflow-y: auto;">
					...
					<p class="txt_p">
						<van-cell
							...
							:key="index"
							:title="item.title"
							:value="item.money / 100 + '元'"
							title-class="cell_title"
							value-class="cell_value"
							:class="index % 2 != 0 ? 'odd_title' : 'even_title'"
						/>
					</p>
					<p class="txt_p">
						徒弟活跃奖励
			...
			<div class="pop_div" id="backAlert" v-if="isBack">
				...
				<div class="btn-container">
					...
					<div class="shareBtn border2" @click="leaveShare">去微信分享赚钱</div>
				</div>
			</div>
			<!-- 离开页面弹窗 end -->
			<!-- 分享弹窗 -->
			<div class="pop_div" id="commonShare" v-if="iscommonShare">
				<img src="../assets/img/invite/close.png" alt="" class="closeBtn" @click="closeBtn()" />
				<p><span class="title">奖励说明</span></p>
				<div class="nine-box">
					<div class="nine-box-1">最低</div>
					<div class="nine-box-2">赚</div>
					<div class="nine-box-3">{{ isFailShare ? tomoney : min }}.00</div>
				...
				<div class="nine-text-1">2.选择{{ share_type ? '微信朋友或者群' : '朋友圈' }}</div>
				<div class="nine-text-2">3.{{ share_type ? '点选好友列表某一好友或群' : '发送朋友圈' }}</div>
				<div class="nine-text-3" v-if="share_type">
					4.点击「分享」到
					<span>3个以上</span>
					微信好友/群，成功邀请机率
					<span>提升200%</span>
				</div>
				<p><span class="button" @click="toShare(0)" v-if="share_type">分享微信朋友或群</span></p>
				<p><span class="button" @click="toShare(1)" v-if="!share_type">分享朋友圈</span></p>
			</div>
			<!-- 分享弹窗 end -->
		...
		<div class="sharePannel" v-if="enterShow">
			...
			<div class="content content1">
				...
				<div class="btn mv" @click="commonShare">立即分享</div>
				<div class="closeBtn" @click="closeEnter"></div>
			</div>
		</div>
		<!-- 进入弹窗 end -->
		<!-- 分享成功弹窗 -->
		<div class="sharePannel" v-if="isShared">
			<div class="sharePannelBg"></div>
			<div class="content content1 shareSuccess">
				<div class="icon iconSuccess"></div>
				<p class="title">分享成功</p>
				<p v-if="share_type" class="desc">
					分享到
					<span style="color:#FF0000">3个</span>
					以上不同的群聊邀请成功率高达
					<span style="color:#FF0000">98%</span>
				</p>
				<p v-if="!share_type" class="desc">
					分享到
					<span style="color:#FF0000">朋友圈</span>
					邀请成功率高达
					<span style="color:#FF0000">98%</span>
				</p>
				<div class="btn mv" @click="commonShare" v-if="share_type">分享微信朋友或群</div>
				<div class="btn mv" @click="commonShare" v-if="!share_type">分享朋友圈</div>
				<div class="closeBtn" @click="closeShare"></div>
			</div>
		</div>
		<!-- 分享成功弹窗 end -->
...
export default {
	...
	data() {
		return {
			...
			iscommonShare: false, //离开页面分享弹窗
			isFailShare: false, //收徒失败分享弹窗
			isShared: false, //分享成功弹窗
	...
	methods: {
		...
		onLoad() {
			...
			common.toAjax(common.host + '/user_st/friend', { last_id: that.last_id, page: that.pageNum }, function(res) {
				// 加载状态结束
				that.loading = false;
				if (res.err_code == 0) {
		...
		closeAll() {
			...
			this.iscommonShare = false; //离开页面分享弹窗
...
.top-input-big {
	background: -webkit-linear-gradient(#fe4540, #d70e25); /* Safari 5.1 - 6.0 */
	background: -o-linear-gradient(#fe4540, #d70e25); /* Opera 11.1 - 12.0 */
	background: -moz-linear-gradient(#fe4540, #d70e25); /* Firefox 3.6 - 15 */
	background: linear-gradient(#fe4540, #d70e25); /* 标准的语法 */
}
...
#commonShare .nine-box {
	margin-top: 20px;
	text-align: center;
}
#commonShare .nine-box .nine-box-1 {
	width: 0.28rem;
	font-size: 14px;
	font-weight: 500;
	color: #ffb095;
...
#commonShare .nine-box .nine-box-2 {
	font-size: 20px;
	font-weight: 500;
	color: #ff490a;
	display: inline-block;
...
#commonShare .nine-box .nine-box-3 {
	font-size: 32px;
	font-weight: 400;
	color: #ff490a;
	display: inline-block;
...
#commonShare .nine-box .nine-box-4 {
	font-size: 24px;
	font-weight: 400;
	color: #ff490a;
	display: inline-block;
...
#commonShare .nine-text span {
	color: #ff490a;
}

#commonShare .nine-text-1 {
	font-size: 15px;
	font-weight: 500;
	color: #333;
}

#commonShare .nine-text-2 {
	font-size: 15px;
	font-weight: 500;
	color: #333;
}

#commonShare .nine-text-3 {
	font-size: 15px;
	width: 70%;
	font-weight: 500;
	text-align: left;
...
.shareSuccess .closeBtn {
	top: 7rem;
}
.inviteGroup .rewardMes {
	width: 2.16rem;
	background: url(../assets/img/invite/dialog.png) no-repeat;
	background-size: 100% 100%;
...
.inviteGroup .rewardMes .beanIcon {
	width: 0.45rem;
	height: 0.35rem;
	background: url(../assets/img/beans.png) no-repeat;
	background-size: 100%;
	margin: 0.05rem auto;
}
.shareSuccess .icon.iconSuccess {
	width: 1.94rem;
	height: 2.12rem;
	background: url(../assets/img/share_success.png) no-repeat;
	background-size: 100%;
	left: 51%;
}
</style>

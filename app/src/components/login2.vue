<template>
	<div class="navTop" :style="bodyH">
		...
		<div class="loginBox box1" :style="loginBoxStyle">
			<p :style="logoP"><img :src="'http://quzhuan.oss-cn-beijing.aliyuncs.com/img/logo/' + logo_pre + '_logo.png'" alt=""
				 :style="logoStyle" /></p>
			<p :style="inputP" v-if="passwordShow">
				...
				<van-cell-group :style="inputStyle" :border="false">
					<van-field v-model="phone" type="number" @blur="checkPhone(phone)" :value="phone" clearable :border="true" @focus="init_Input_tel(1)"
					 style="padding-left: 0;" ref="tel"></van-field>
				</van-cell-group>
			</p>
		...
		<div class="loginBox box2" :style="loginBoxStyle">
			...
			<p :style="inputP">
				...
				<van-cell-group :style="inputStyle" :border="false">
					<van-field v-model="phone" type="number" :value="phone" clearable :border="true" readonly style="padding-left: 0;"
					 ref="tel"></van-field>
				</van-cell-group>
			</p>
			<p :style="inputP" v-if="!ispwdLogin">
				...
				<van-cell-group :style="inputStyle" :border="false">
					<van-field v-model="new_pass" :type="type_new" :right-icon="icon_new" :error="new_pass_confirm != '' && new_pass == '' && new_pass.length >= 6"
					 :error-message="errMsg" @click-right-icon="showIco2" @keyup="checkConfirm(new_pass, new_pass_confirm)" clearable
					 @focus="init_Input_pass(1)" style="padding-left: 0;" ref="pass"></van-field>
				</van-cell-group>
			</p>
			<p :style="inputP" v-if="!ispwdLogin">
				<span class="inputLabel label-pass-confirm" :style="inputLabelStyle" @click="init_Input_pass_confirm(0)">请再次输入新密码</span>
				<van-cell-group :style="inputStyle" :border="false">
					<van-field v-model="new_pass_confirm" :type="type_new_confirm" :right-icon="icon_new_confirm" @click-right-icon="showIco3"
					 :error="new_pass_confirm == '' && new_pass != '' && new_pass_confirm.length >= 6" :error-message="errMsg_confirm"
					 @keyup="checkConfirm(new_pass, new_pass_confirm)" @focus="init_Input_pass_confirm(1)" style="padding-left: 0;"
					 ref="pass_confirm"></van-field>
				</van-cell-group>
			</p>
			<p :style="inputP" v-if="ispwdLogin">
				<span class="inputLabel label-password" :style="inputLabelStyle" @click="init_Input_password(0)">请输入密码</span>
				<van-cell-group :style="inputStyle" :border="false">
					<van-field v-model="password" :type="type_password" :right-icon="icon_password" @click-right-icon="showIco4"
					 clearable @focus="init_Input_password(1)" style="padding-left: 0;" ref="password"></van-field>
				</van-cell-group>
			</p>
			<p :style="btnP">
	...
	export default {
		...
		methods: {
			...
			toLogin: function(type) {
				...
				switch (type) {
					case 0:
						...
						// }
						if (that.phone != '' && that.new_pass != '' && that.new_pass_confirm != '' && that.new_pass == that.new_pass_confirm &&
							that.canLogin) {
							common.toAjax(
			...
			checkConfirm(new_pass, new_pass_confirm) {
				...
				if (new_pass != '' && new_pass_confirm != '' && new_pass == new_pass_confirm && new_pass.length >= 6 &&
					new_pass_confirm.length >= 6) {
					this.errMsg = '';
					this.errMsg_confirm = '';
					this.canLogin = true;
				}
...
<style scoped>
	...
	.inputBtn {
		...
		color: #ffffff;
		/* 橙色主题*/
		background: -webkit-linear-gradient(#fe8542, #ec5c43);
		/* Safari 5.1 - 6.0 */
		background: -o-linear-gradient(#fe8542, #ec5c43);
		/* Opera 11.1 - 12.0 */
		/* Firefox 3.6 - 15 */
		background: -webkit-gradient(linear, left top, left bottom, from(#fe8542), to(#ec5c43));
		background: linear-gradient(#fe8542, #ec5c43);
		/* 标准的语法 */
		-webkit-box-shadow: 1px 1px 10px #ff4500;
	...
	.loginBox.box2 {
		top: 52%;
		-webkit-transform: translate3d(-50%, -50%, 0) scalex(0.9);
	...
	.loginBox.box3 {
		top: 54%;
		-webkit-transform: translate3d(-50%, -50%, 0) scalex(0.8);
	...
	.loginBox.box4 {
		top: 56%;
		-webkit-transform: translate3d(-50%, -50%, 0) scalex(0.7);
	...
	.box2tobox1 {
		-webkit-animation: tobox1 0.7s ease-in-out;
		animation: tobox1 0.7s ease-in-out;
	}

	.box3tobox2 {
		-webkit-animation: tobox2 0.7s ease-in-out;
		animation: tobox2 0.7s ease-in-out;
	}

	.box4tobox3 {
		-webkit-animation: tobox3 0.7s ease-in-out;
		animation: tobox3 0.7s ease-in-out;
	}

	@-webkit-keyframes tobox1 {
		from {
			top: 52%;
			-webkit-transform: translate3d(-50%, -50%, 0) scalex(0.9);
			transform: translate3d(-50%, -50%, 0) scalex(0.9);

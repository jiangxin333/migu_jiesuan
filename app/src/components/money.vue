<template>
	<div>
		...
		<van-tabs v-model="active" swipeable sticky animated color="#5da968" :line-width="20" :line-height="3" :offset-top="0"
		 @change="changeActive">
			<van-tab v-for="(item, index) in menus" :key="index" :title="item.name">
				<div style="padding-bottom: 60px;position: relative;">
					<van-list :v-model="'loading'" :finished="finished" finished-text="没有更多了" :loading="checkLoading" @load="onLoad"
					 :offset="300" :immediate-check="isImmCheck">

						<van-pull-refresh v-model="isLoading" @refresh="onRefresh" v-if="!init">
							<div class="listCard" v-for="listitem in listArr" :key="listitem.index" :id="activeIndex" @click="goArticle(listitem.article_id, listitem.is_video)">
								<van-row>
									<van-col span="24">
										<van-row type="flex" justify="space-between">
											<van-col span="16">
												...
												<van-row type="flex" align="bottom">
													...
													<van-col span="18">
														<span class="redTxt" style="float: right;margin-right: 10px;">
...
<script>
	...
	export default {
		...
		methods: {
			...
			toWeb() {
				var picurl =
					"https://ss1.baidu.com/6ONXsjip0QIZ8tyhnq/it/u=3380362153,2833752586&fm=85&app=57&f=PNG?w=121&h=75&s=E4C2F012C63F718A326419CF0300C030"

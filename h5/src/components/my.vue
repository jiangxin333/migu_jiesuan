<template>
	<div class="navTop myTop" style="padding-bottom: 60px;">
		<div id="download" @click="download" style="height: 40px;line-height: 40px;font-size: 14px;color:#FFA77C;background:#FFFFF0;border-bottom: 1px solid #DDDDDD;"
		 v-if="en_top == 1">
		...
		<div id="cover" style="margin-top: 65px;">
		...
		<div class="swiper-container">
			<div class="swiper-wrapper">
				<div class="swiper-slide" v-for="(item, index) in adImgList" :key="index"><img :src="item.img" alt @click="toAd(item.url)"
					 style="width: 100%;" /></div>
			</div>
		</div>
	...
	export default {
		...
		activated() {
			...
			if (common.getVal('loginData') != null) {
				...
				this.en_top = common.getVal('loginData').switch.en_top;
				this.app_download_title = common.getVal('loginData').app_download.app_download_title;
				...
				common.toAjax(common.host + '/api/app/commoninfo', {}, function(res) {
					...
					that.en_top = res.data.switch.en_top;
					that.app_download_title = res.data.app_download.app_download_title;
					if (common.getVal('loginData').ad_info.length > 0) {
						this.ad_show = true;
						this.adImgList = common.getVal('loginData').ad_info
						this.$nextTick(() => {
...
<style scoped>
	...
	.top_user_img span {
		...
		/* Opera 11.1 - 12.0 */
		background: -moz-linear-gradient(#c8ffcb, #98e59d);
		/* Firefox 3.6 - 15 */

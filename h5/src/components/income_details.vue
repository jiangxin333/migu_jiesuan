				...
				<van-tab title="文章分享">
					<div class="list_content" :style="{ height: listH }">
						...
						<van-pull-refresh v-model="isLoading_article" @refresh="onRefresh_articleList" loading-text="正在刷新数据">
							<van-list v-model="article_loading" :finished="article_finished" finished-text="文章分享仅供最近3天记录查询" @load="getData_articleList(false)"
							 :immediate-check="true">
								<div class="articleList" v-for="(article, index) in articleList" :key="index">
									<p>{{ article.key }}</p>
									<div class="articleCard box_show" v-for="articleinfo in article.money_list" :key="articleinfo.key" :id="articleinfo.id"
				...
				<van-tab title="师徒奖励">
					<div class="list_content" :style="{ height: listH, 'padding-top': '20px' }">
						...
						<van-pull-refresh v-model="isLoading_mentor" @refresh="onRefresh_mentorList" loading-text="正在刷新数据">
							<van-list v-model="mentor_loading" :finished="mentor_finished" finished-text="师徒奖励仅供最近3天记录查询" @load="getData_mentorList(false)"
				...
				<van-tab title="其他收入">
					<div class="list_content" :style="{ height: listH }">
						...
						<van-pull-refresh v-model="isLoading_other" @refresh="onRefresh_otherList" loading-text="正在刷新数据">
							<van-list v-model="other_loading" :finished="other_finished" finished-text="其他收入仅供最近3天记录查询" @load="getData_otherList(false)"
							 :immediate-check="false">
								<!-- 这里 -->
								...
								<van-collapse v-model="activeNames" accordion>
									<van-collapse-item v-for="(other,index) in otherList" :title="other.title" :value="other.updated_at" :key="index"
									 :name="index">
										<p class="otherMessage">{{ other.desc }}</p>
									</van-collapse-item>
								</van-collapse>
...
<script>
	...
	export default {
		...
		methods: {
			...
			getData_articleList(isRefresh) {
				...
				if (isRefresh) {
					that.isRequest = true;
					that.last_id_1 = 0;
					common.toAjax(common.host + '/users_money/income', {
						type: 1,
						is_refresh: 1,
						last_id: that.last_id_1
					}, function(res) {
		...
		mounted() {
			this.listH = window.innerHeight - $('.van-nav-bar').height() - $('.van-row').height() - $('.van-tabs__wrap').height() +
				'px';
		}
	};
</script>

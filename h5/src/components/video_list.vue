<template>
	<div style="background: #FFFFFF;padding-bottom: 50px;">
		...
		<van-pull-refresh v-model="isLoading" @refresh="onRefresh">
			<van-list :v-model="'loading'" :finished="finished" :loading="checkLoading" finished-text="没有更多了" @load="getVideoList" :offset="300">
				<div
					class="listCard"
	...
	methods: {
		getVideoList() {
			...
			if (that.isRequest) {
				...
				$.ajax({
					url: common.host + '/article/list?type=0&page=' + that.pageNum,
					type: 'GET',
					success(res) {
						that.loading = false;
						that.isRequest = true;

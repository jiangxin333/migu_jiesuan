...
export default {
	 ...
	 */
	login(that) {
		var tthat = this; //common.js this对象
		api.addEventListener({
			...
			tthat.toAjax(tthat.host + '/applogin', {
				code: ret.value.code
			}, function(res) {
				if (res.err_code == 0) {
	...
	*/
	delayHide(that){
		...
		that.time3=setTimeout(function(){
			wx.hideAllNonBaseMenuItem();
		},2000)
		that.time4=setTimeout(function(){
			wx.hideAllNonBaseMenuItem();
		},4000)
	},
	/**
	...
	*/
	clearHide(that){
		...
		clearTimeout(that.time3);
		clearTimeout(that.time4);
	},
	/**
	 * 时间戳格式化函数 
	 * @date 2019-02-13 
